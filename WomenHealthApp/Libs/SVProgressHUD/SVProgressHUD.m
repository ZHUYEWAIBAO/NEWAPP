...
+ (void)showSuccessWithStatus:(NSString *)string {
    //UIImage *aimg = [UIImage imageNamed:@"SVProgressHUD.bundle/success.png"];
    
    //[self showImage:[UIImage imageWithContentFileName:@"SVProgressHUD.bundle/success.png"] status:string];
    [self showImage:[UIImage imageNamed:@"SVProgressHUD.bundle/success.png"] status:string];
}

+ (void)showErrorWithStatus:(NSString *)string {
    //[self showImage:[UIImage imageWithContentFileName:@"SVProgressHUD.bundle/error.png"] status:string];
...
+ (void)dismissWithStatus:(NSString*)newStatus afterDelay:(NSTimeInterval)interval {
    [SVProgressHUD dismissWithStatus:newStatus afterDelay:interval endBlock:nil];
}


+ (void)dismissWithStatus:(NSString*)newStatus afterDelay:(NSTimeInterval)interval endBlock:(void (^)()) block{

    [[self sharedView] setStatus:newStatus];
...
- (void)showImage:(UIImage *)image status:(NSString *)string duration:(NSTimeInterval)duration {
    ...
    self.fadeOutTimer = [NSTimer timerWithTimeInterval:duration target:self selector:@selector(dismissSuccess) userInfo:nil repeats:NO];
...
-(void)dismissSuccess
{
    ...
    [UIView animateWithDuration:0.15
                     ...
                     animations:^{
                         weakSelf.hudView.transform = CGAffineTransformScale(self.hudView.transform, 1.0, 1.0);
                         weakSelf.alpha = 0;
                     }
                     completion:^(BOOL finished){
                         if(weakSelf.alpha == 0) {
